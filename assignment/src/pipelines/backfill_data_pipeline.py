...
def run_pipeline(start_date_str: str, end_date_str: str):
    # subtract 6 months from start_date to account for feature lag
    start_date_obj = datetime.strptime(start_date_str, "%Y-%m-%d") - relativedelta(months=6)
...
def main():
    ...
    parser.add_argument("--skip-backfill", action="store_true", help="Skip backfilling the bronze/silver/gold tables.")
    args = parser.parse_args()
    print(args)
    if not args.skip_backfill:
        run_pipeline(args.start_date, args.end_date)
    else:
        print("Skipping backfill as per the --skip-backfill flag.")
