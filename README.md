...
2.  **Build and Run**: cd to `assignment/docker/airflow` directory in your terminal and run this command. It will build the Docker image and start up Airflow.
    ```bash
...
3.  You can customize the run using the "Trigger DAG w/ config" option. Key parameters include:
    *   `skip_backfill`: For the first run, set to `false`. Set to `true` to skip the data generation steps if the data already exists.
    *   `stage1_trials`, `stage2_trials`: Number of Optuna trials for hyperparameter tuning. Advise to keep at 5 for demo purposes.
    *   `k_folds`: Number of folds for cross-validation during training. Advise to keep at 5 for demo purposes.
    *   `bronze_start`: keep it at 2023-07-01  (for features, it will take 6 months back so it will capture 2023-01-01 data as well)
    *   `bronze_end`: keep it at 2024-09-01
...
*   **`use_latest_model`**: If `true` (default), the DAG will automatically find and use the most recently trained model from the `assignment/outputs/model_tuning/` directory.
*   **`alert_email`**: The email address where the simulated alert should be sent. No need to change for demo purpose.
